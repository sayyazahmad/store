...
namespace SmartStore.PayTabs.Services
{
	public class PayTabsService : IPayTabsService
	{
			...
			List<Dictionary<string, object>> items)
		{
			...
			if (!includingTax)
			{
				...
				// In a B2C scenario, where taxes are included, no taxes should be submitted to PayTabs."
			...
			if (total != itemsPlusMisc)
			{
				...
				if (items != null && otherAmount != decimal.Zero)
				{
					...
					var line = new Dictionary<string, object>();
					line.Add("quantity", "1");
					line.Add("name", T("Plugins.SmartStore.PayTabs.Other").Text.Truncate(127));
        ...
        public PayTabsResponse EnsureAccessToken(PayTabsSessionData session, PayTabsApiSettingsBase settings)
        {
        ...
        private string ToInfoString(dynamic json)
		{
			...
			try
			{
				string[] strings = T("Plugins.SmartStore.PayTabs.MessageStrings").Text.SplitSafe(";");
				...
				sb.AppendLine("{0}: {1}".FormatInvariant(strings.SafeGet((int)PayTabsMessage.Message), message.NaIfEmpty()));
				sb.AppendLine("{0}: {1}".FormatInvariant(strings.SafeGet((int)PayTabsMessage.Event), eventType.NaIfEmpty()));
				sb.AppendLine("{0}: {1}".FormatInvariant(strings.SafeGet((int)PayTabsMessage.EventId), eventId.NaIfEmpty()));
				sb.AppendLine("{0}: {1}".FormatInvariant(strings.SafeGet((int)PayTabsMessage.PaymentId), paymentId.NaIfEmpty()));
				sb.AppendLine("{0}: {1}".FormatInvariant(strings.SafeGet((int)PayTabsMessage.State), state.NaIfEmpty()));
				sb.AppendLine("{0}: {1}".FormatInvariant(strings.SafeGet((int)PayTabsMessage.Amount), amount.NaIfEmpty()));
			}
			catch { }
		...
		{
			return sandbox ? "https://api.sandbox.PayTabs.com" : "https://api.PayTabs.com";
		}

		public void AddOrderNote(PayTabsSettingsBase settings, Order order, string anyString, bool isIpn = false)
		{
			try
			{
				...
				string[] orderNoteStrings = T("Plugins.SmartStore.PayTabs.OrderNoteStrings").Text.SplitSafe(";");
		...
		public PayTabsPaymentInstruction ParsePaymentInstruction(dynamic json)
		{
			if (json == null)
				return null;

			DateTime dt;
			var result = new PayTabsPaymentInstruction();
			...
			try
			{
				...
				if (rbi != null)
				{
					result.RecipientBanking = new PayTabsPaymentInstruction.RecipientBankingInstruction();
		...
		public string CreatePaymentInstruction(PayTabsPaymentInstruction instruct)
		{
			...
			var instructStrings = T("Plugins.SmartStore.PayTabs.PaymentInstructionStrings").Text.SplitSafe(";");

			if (instruct.IsManualBankTransfer)
			{
				sb.AppendFormat(paragraphTemplate, T("Plugins.SmartStore.PayTabs.ManualBankTransferNote"));

				sb.AppendFormat(rowTemplate, instructStrings.SafeGet((int)PayTabsPaymentInstructionItem.Reference), instruct.ReferenceNumber);
				sb.AppendFormat(rowTemplate, instructStrings.SafeGet((int)PayTabsPaymentInstructionItem.AccountNumber), instruct.RecipientBanking.AccountNumber);
				sb.AppendFormat(rowTemplate, instructStrings.SafeGet((int)PayTabsPaymentInstructionItem.AccountHolder), instruct.RecipientBanking.AccountHolderName);

				sb.AppendFormat(rowTemplate, instructStrings.SafeGet((int)PayTabsPaymentInstructionItem.Bank), instruct.RecipientBanking.BankName);
				sb.AppendFormat(rowTemplate, instructStrings.SafeGet((int)PayTabsPaymentInstructionItem.Iban), instruct.RecipientBanking.Iban);
				sb.AppendFormat(rowTemplate, instructStrings.SafeGet((int)PayTabsPaymentInstructionItem.Bic), instruct.RecipientBanking.Bic);
			}
			else if (instruct.IsPayUponInvoice)
			{
				...
				var intro = T("Plugins.SmartStore.PayTabs.PayUponInvoiceLegalNote", _companyInfoSettings.Value.CompanyName.NaIfEmpty());

				// /v1/payments/payment/<id>/payment-instruction not working anymore. Serves 401 unauthorized.
				//if (instruct.Link.HasValue())
				//{
				//	intro = "{0} <a href='{1}'>{2}</a>.".FormatInvariant(intro, instruct.Link, instructStrings.SafeGet((int)PayTabsPaymentInstructionItem.Details));
				//}
				...
				sb.AppendFormat(rowTemplate, instructStrings.SafeGet((int)PayTabsPaymentInstructionItem.Bic), instruct.RecipientBanking.Bic);
				sb.Append("<br />");
				sb.AppendFormat(rowTemplate, instructStrings.SafeGet((int)PayTabsPaymentInstructionItem.Amount), amount);
				if (instruct.DueDate.HasValue)
				{
					sb.AppendFormat(rowTemplate, instructStrings.SafeGet((int)PayTabsPaymentInstructionItem.PaymentDueDate), instruct.DueDate.Value.ToString("d", culture));
				}
				sb.AppendFormat(rowTemplate, instructStrings.SafeGet((int)PayTabsPaymentInstructionItem.Reference), instruct.ReferenceNumber);
			}
		...
		public PayTabsResponse CallApi(string method, string path, string accessToken, PayTabsApiSettingsBase settings, string data)
		{
			var isJson = (data.HasValue() && (data.StartsWith("{") || data.StartsWith("[")));
			var encoding = (isJson ? Encoding.UTF8 : Encoding.ASCII);
			var result = new PayTabsResponse();
			HttpWebResponse webResponse = null;

			var url = GetApiUrl(false) + path.EnsureStartsWith("/");
			...
			{
				Logger.Error(T("Plugins.SmartStore.PayTabs.MissingAccessToken", method.NaIfEmpty(), path.NaIfEmpty()));
			}
			
			request.Headers["PayTabs-Partner-Attribution-Id"] = "SmartStoreAG_Cart_PayTabsPlus";
		...
		public PayTabsResponse GetPayment(PayTabsApiSettingsBase settings, PayTabsSessionData session)
		{
		...
		public PayTabsResponse CreatePayment(
			...
			string cancelUrl)
		{
			...
			// payer, payer_info
			// PayTabs review: do not transmit
			...
			payer.Add("payment_method", "PayTabs");
			...
			//Logger.InsertLog(LogLevel.Information, "PayTabs PLUS", JsonConvert.SerializeObject(data, Formatting.Indented) + "\r\n\r\n" + (result.Json != null ? result.Json.ToString() : ""));

			return result;
		}

		public PayTabsResponse PatchShipping(
			PayTabsApiSettingsBase settings,
			PayTabsSessionData session,
			List<OrganizedShoppingCartItem> cart,
			string providerSystemName)
		...
		public PayTabsResponse ExecutePayment(PayTabsApiSettingsBase settings, PayTabsSessionData session)
		{
			var data = new Dictionary<string, object>();
		...
		public PayTabsResponse Refund(PayTabsApiSettingsBase settings, PayTabsSessionData session, RefundPaymentRequest request)
		{
			var data = new Dictionary<string, object>();
			...
			//Logger.InsertLog(LogLevel.Information, "PayTabs Refund", JsonConvert.SerializeObject(data, Formatting.Indented) + "\r\n\r\n" + (result.Json != null ? result.Json.ToString() : ""));

			return result;
		}

		public PayTabsResponse Capture(PayTabsApiSettingsBase settings, PayTabsSessionData session, CapturePaymentRequest request)
		{
			var data = new Dictionary<string, object>();
		...
		public PayTabsResponse Void(PayTabsApiSettingsBase settings, PayTabsSessionData session, VoidPaymentRequest request)
		{
		...
		public PayTabsResponse CreateWebhook(PayTabsApiSettingsBase settings, PayTabsSessionData session, string url)
		{
			var data = new Dictionary<string, object>();
		...
		public PayTabsResponse DeleteWebhook(PayTabsApiSettingsBase settings, PayTabsSessionData session)
		{
		...
		/// <remarks>return 503 (HttpStatusCode.ServiceUnavailable) to ask PayTabs to resend it at later time again</remarks>
			...
			string providerSystemName)
		{
			...
			// validating against PayTabs SDK failing using sandbox, so better we do not use it:
			//var apiContext = new global::PayTabs.Api.APIContext
			//{
			...
			//		}
			//};
			//var result = global::PayTabs.Api.WebhookEvent.ValidateReceivedEvent(apiContext, headers, rawJson, webhookId);
			//}
			...
			if (paymentId.IsEmpty())
			{
				Logger.Log(
					...
					T("Plugins.SmartStore.PayTabs.FoundOrderForPayment", 0, "".NaIfEmpty()),
					null);
			...
			if (orders.Count != 1)
			{
				Logger.Log(
					LogLevel.Warning,
					new Exception(JsonConvert.SerializeObject(json, Formatting.Indented)),
					T("Plugins.SmartStore.PayTabs.FoundOrderForPayment", orders.Count, paymentId),
					null);
			...
			if (!primaryCurrency.IsCaseInsensitiveEqual(currency))
			{
				Logger.Log(
					LogLevel.Warning,
					new Exception(JsonConvert.SerializeObject(json, Formatting.Indented)),
					T("Plugins.SmartStore.PayTabs.CurrencyNotEqual", currency.NaIfEmpty(), primaryCurrency),
					null);
	...
	[Serializable]
	public class PayTabsSessionData
	{
		public PayTabsSessionData()
		{
		...
		public PayTabsPaymentInstruction PaymentInstruction { get; set; }
	...
	[Serializable]
	public class PayTabsPaymentInstruction
	{
