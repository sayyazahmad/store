...
namespace SmartStore.Admin.Controllers
{
    ...
    public partial class CustomerController : AdminControllerBase
    {
		...
		protected virtual void PrepareCustomerModelForCreate(CustomerModel model)
		{
            ...
            if (_customerSettings.IsAgentEnabled)
            {
                model.AvailableMembershipPlans.Add(new SelectListItem { Text = T("Admin.Address.SelectMembershipPlan"), Value = "0" });
                foreach (var c in _membershipPlanService.GetAll())
                {
                    model.AvailableMembershipPlans.Add(new SelectListItem
                    {
                        Text = c.Title,
                        Value = c.Id.ToString(),
                        Selected = c.Id == model.MembershipPlanId
                    });
                }
            }
        }
        ...
        public ActionResult Edit(CustomerModel model, bool continueEditing, FormCollection form)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    customer.MembershipPlanId = model.MembershipPlanId;
        ...
        public ActionResult BankUpdateRequest()
        {
            ...
            var requests = _bankUpdateRequestService.GetAllBankUpdateRequests();
            var model = requests.Select(x => ToModel(x));
            return View(model);
        }

        public ActionResult ApproveBankUpdateRequest(int id)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedView();

            UpdateRequestStatus(id, 20);
        ...
        public ActionResult RejectBankUpdateRequest(int id)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedView();

            UpdateRequestStatus(id, 30);
        ...
        private string GetStatus(int requestStatusId)
        {
