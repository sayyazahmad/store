...
namespace SmartStore.Admin.Controllers
{
    ...
    public partial class StoreController : AdminControllerBase
    {
        ...
        private IEnumerable<BannerImageModel> PopulateModel(int id)
        {
            var bannerImages = Services.StoreService.GetStoreBannerImagesByStoreId(id);
            return bannerImages.Select(x => x.ToModel());
        }

        public ActionResult DeleteBannerImage(int id)
        {
            if (!Services.Permissions.Authorize(StandardPermissionProvider.ManageStores))
                return AccessDeniedView();

            var img = Services.StoreService.GetGetBannerImageIdById(id);
            if(!string.IsNullOrEmpty(img?.URL))
            {
                string path = Path.Combine(Server.MapPath("~/content/images"), img.URL);
                if (System.IO.File.Exists(path))
                {
                    System.IO.File.Delete(path);
                }
                Services.StoreService.DeleteBannerImage(id);
                NotifySuccess("Image Deleted");
                return RedirectToAction("BannerImages", new { id = img.StoreId });
            }
            return View();
        }
